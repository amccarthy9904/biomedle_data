...
for i in range(uniqueADM0Codes.size().toInt().getInfo()):
    ...
    countryName = country.get('ADM0_NAME').getInfo()
    if countryName in country_names:
        x += 1
        # print(f"{i} {countryName}")
print(x)
        
    # countryName = country.get('ADM0_NAME').getInfo().replace(' ', '')
    # print(f"{i} {countryName}")


for i in range(15):
    adm0Code = ee.Number(uniqueADM0Codes.get(i))
    print(adm0Code.getInfo())
    country = countries.filter(ee.Filter.eq('ADM0_CODE', adm0Code)).first()
    ...
    fileName = ee.String(countryName).cat('_').cat(areaString)
    ...
    with open(os.path.join(temp_directory, f'{fileName.getInfo()}.zip'), 'wb') as f:
        f.write(response.content)

    # Extract the contents of the zip file
    subprocess.run(['unzip', '-o', os.path.join(temp_directory, f'{fileName.getInfo()}.zip'), '-d', temp_directory])
